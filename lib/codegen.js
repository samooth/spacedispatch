...
module.exports = function generateCode (spacedispatch, { esm = false }) {
  let str = ''
  str += '// This file is autogenerated by the spacedispatch compiler\n'
  str += '/* eslint-disable camelcase */\n'
  str += '\n'

  if (esm) {
    str += 'import { c, b4a, assert } from \'spacedispatch/runtime\'\n'
    str += 'import { version, getEncoding, setVersion } from \'./messages.js\'\n'
  } else {
    str += 'const { c, b4a, assert } = require(\'spacedispatch/runtime\')\n'
  ...
  str += 'class Router {\n'
  str += '  constructor () {\n'
  for (const handler of spacedispatch.handlers) {
    str += `    this._handler${handler.id} = null\n`
  }
  str += '\n'
  str += `    this._missing = ${spacedispatch.handlers.length}\n`
  str += '  }\n'
  str += '\n'
