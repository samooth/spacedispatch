...
module.exports = function generateCode (hyperswitch) {
  ...
  str += '  _checkAll () {\n'
  for (const handler of hyperswitch.handlers) {
    str += `    if (!this._handler${handler.id}) throw new Error('Missing handler for: ' + ${s(handler.name)})\n`
  }
  str += '  }\n'
  str += '\n'
  ...
  str += '\n'
  str += '  const id = getId(name)\n'
  str += '  const enc = getEncoder(name, opts)\n'
  str += '\n'
  str += '  c.uint.preencode(state, id)\n'
  str += '  enc.preencode(state, message)\n'
  str += '\n'
  str += '  state.buffer = b4a.allocUnsafe(state.end)\n'
  str += '  c.uint.encode(state, id)\n'
  ...
  str += 'function getId (name) {\n'
  str += '  switch (name) {\n'
  for (const handler of hyperswitch.handlers) {
    str += `    case ${s(handler.name)}:\n`
    str += `      return ${handler.id}\n`
  }
  str += '    default:\n'
  ...
  str += 'function getEncoder (name, opts) {\n'
  str += '  switch (name) {\n'
  for (const handler of hyperswitch.handlers) {
    str += `    case ${s(handler.name)}:\n`
    str += `      return resolveStruct(${s(handler.requestType)}, opts && opts.version)\n`
  }
  str += '    default:\n'
  str += '      throw new Error(\'Handler not found for name:\' + name)\n'
  str += '  }\n'
